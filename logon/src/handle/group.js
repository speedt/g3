...
(() => {
  ...
  exports.re_entry = function(send, msg){
    if(!_.isString(msg.body)) return logger.error('group re_entry empty');
  ...
  function p2(send, data, err){
    if('string' !== typeof err) return logger.error('group re_entry:', err);

    var _data = [];
    _data.push(data.channelId);
    _data.push(JSON.stringify([3010, data.seqId, _.now(), , err]));
