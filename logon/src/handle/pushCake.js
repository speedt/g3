...
(() => {
   ...
   * @return
   */
  exports.switchSeat = function(send, msg){
    try{ var data = JSON.parse(msg);
    }catch(ex){ return; }

    biz.pushCake.switchSeat(data.serverId, data.channelId)
  ...
  function p1(send, data, doc){
    ...
    var _data = [
      null,
      JSON.stringify([5018, doc[1], _.now(), data.seqId]),
    ];
    ...
    for(let i of _.values(doc[0])){
      ...
      send('/queue/back.send.v3.'+ i.server_id, { priority: 9 }, _data, err => {
        if(err) return logger.error('pushCake switchSeat:', err);
      });
    }
  }

  function p2(send, data, err){
    if('object'   === typeof err) return logger.error('pushCake switchSeat:', err);
    if('invalid_user_id' === err) return logger.debug('pushCake switchSeat:', err);

    var _data = [
      data.channelId,
      JSON.stringify([5018, , _.now(), data.seqId, err]),
    ];
